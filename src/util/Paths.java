...
/**
 * Resolves project-relative paths against this class's containing JAR
 * (or compile-output directory), not the process CWD.
 *
 * Mirrors DESIGN.md §1: anchor every path on the file's own location,
 * not on wherever {@code java} happened to be invoked from.
 *
 * At runtime {@code getProtectionDomain().getCodeSource().getLocation()}
 * points at:
 * <ul>
 *   <li>the JAR file itself, when launched via {@code java -jar PekkaBot.jar}</li>
 *   <li>the compile-output directory, when launched from an IDE</li>
 * </ul>
 * Either way, its parent directory is the project root that contains
 * {@code data/} and {@code libs/} at deployment time.
 */
